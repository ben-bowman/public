...
- The query returns individual order details from the "Order Details" table,
  along with OrderDate, ProductID, ProductName, net_price (after applying discount),
  total_quantity_sold, and CategoryName.
- A product must have at least a certain number of records (default relaxed to 10)
...
# -------------------------------
DEFAULT_MIN_RECORDS_THRESHOLD = 10  # Relaxed threshold (default is now 10)
...
def main():
    ...
    if insufficient_products:
        df_insuff = df[df['ProductID'].isin(insufficient_products)]\
    ...
    if not recommendations_df.empty:
        ...
        # Retrieve and display the recommended price for the selected product
        ...
        # Display elasticity at current price with interpretation
